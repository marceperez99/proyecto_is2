...
def item_factory(data):
    ...
    tipo = TipoDeItem.objects.get(nombre=data['tipo'])
    item = Item.objects.create(tipo_de_item=tipo, estado=data['estado'], codigo=data['codigo'])
    if 'encargado_de_modificar' in data.keys():
        item.encargado_de_modificar = Participante.objects.get(proyecto=tipo.fase.proyecto, usuario__username=data['encargado_de_modificar'])
    if 'estado_anterior' in data.keys():
        item.estado_anterior = data['estado_anterior']

    item.save()
    for version, datos in data['versiones'].items():
